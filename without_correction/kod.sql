...
CREATE TABLE Nieruchomosci(
  ...
  zrodlo_energii Clob NOT NULL
        CHECK (zrodlo_energii  in   ('brak','wlasne','elektrownia')),
  Hipoteka Char(1 ),
...
Nieruchomości należące do spółdzielni
/
...
Domy - nieruchomości należące do spółdzielni
/
...
Mieszkańcy należący do spółdzielni mieszkaniowej
/
...
CREATE INDEX IX_Relationship8 ON Mieszkancy (nr_adresu)
/
...
Części wspólne należące do spółdzileni mieszkaniowej
/
...
-- Table Nieruchomosc_Pracownik

CREATE TABLE Nieruchomosc_Pracownik(
  nr_nieruchomosci Varchar2(100 ) NOT NULL,
  nr_pracownika Integer NOT NULL
)
/
...
CREATE INDEX IX_posiada ON Wlasciciele (Nr_Spoldzielni_Mieszkaniowej)
Spółdzilenia mieszkaniowa posiada właściciela
/
...
ALTER TABLE Mieszkancy ADD CONSTRAINT Mieszkaniec_ma_adres FOREIGN KEY (nr_adresu) REFERENCES Adresy (Nr_adresu)
/
